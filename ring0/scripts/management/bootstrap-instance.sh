...
function prepare() {
	...
	INSTANCE_BOOTSTRAP_IPADDR="$(echo "$INSTANCE_MANAGEMENT_BOOTSTRAP_IPADDR_CIDR" | awk -F/ '{print $1}')"
	export INSTANCE_BOOTSTRAP_IPADDR
...
function install_ipxe() {
	...
	if ! incus storage volume list default | grep -q ipxe; then
		# SC2029: $URL is intentionally expanded client-side
...
function deploy_instance() {
	...
	if ! incus list "$NAME" -f yaml | grep -q "name:"; then
		...
		# Add a data disk
		if ! incus storage volume info default "$DATA_DISK" >/dev/null 2>&1; then
...
function create_talos_config() {
	...
	instance_bootstrap_ipaddr="$(echo "$INSTANCE_MANAGEMENT_BOOTSTRAP_IPADDR_CIDR" | awk -F/ '{print $1}')"
	local instance_services_ipaddr
	instance_services_ipaddr="$(echo "$INSTANCE_MANAGEMENT_SERVICES_IPADDR_CIDR" | awk -F/ '{print $1}')"
...
function bootstrap_kubernetes() {
	...
	local MANAGEMENT_IPADDR
	MANAGEMENT_IPADDR="$(incus list | awk '/management/ {print $6}' | grep -F ".")"
	...
	# Use an array so flags are properly word-split by the shell
	local -a talos_opts=(-n management -e management "--talosconfig=$RING0_ROOT/dist/talosconfig")
