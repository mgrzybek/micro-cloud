...
if [[ -z "${BRIDGE_BOOTSTRAP_NAME:-}" ]]; then
	...
	if ! incus network show "$BRIDGE_BOOTSTRAP_NAME" >/dev/null 2>&1; then
...
if [[ -z "${TALOS_FACTORY_UUID:-}" ]]; then
	echo "TALOS_FACTORY_UUID must be defined"
	exit 1
fi
...
function prepare() {
	...
	export TLD
	TLD="$(tailscale dns status | awk '/MagicDNS:/ {gsub(")","") ; print $NF}')"
	if [[ -z "$TLD" ]]; then
	...
	export SERVER_ADDR
	SERVER_ADDR="$(echo "$IFACE_BOOTSTRAP_IPADDR_CIDR" | awk -F/ '{print $1}')"
...
function create_instance() {
	...
	if ! incus list "$INSTANCE" -f yaml | grep -q "name:"; then
		incus launch -v images:debian/12 "$INSTANCE"
